...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['accion'])) {
    ...
    // R18: entrada — no duplicar si ya hay entrada sin salida
...
$stmt = $db->prepare("
    SELECT e.*, p.Nombres, p.Apellidos, p.Email, p.Telefono, p.Numero_Documento,
           c.Nombre_Cargo, c.Salario_Base, c.Hora_Inicio_Jornada, c.Hora_Fin_Jornada,
...
// R13: contrato vigente del empleado
$stContrato = $db->prepare("
    SELECT * FROM CONTRATOS
    WHERE ID_Empleado=? AND Estado_Contrato='Vigente'
    ORDER BY Fecha_Inicio DESC LIMIT 1
");
$stContrato->execute([$empId]);
$contratoVigente = $stContrato->fetch();

// R17: días laborales del cargo
$stDias = $db->prepare("SELECT Dia_Semana FROM CARGO_DIAS WHERE ID_Cargo=? ORDER BY FIELD(Dia_Semana,'Lunes','Martes','Miércoles','Jueves','Viernes','Sábado','Domingo')");
$stDias->execute([$emp['ID_Cargo']]);
$diasLaborales = $stDias->fetchAll(PDO::FETCH_COLUMN);
...
// ── Historial ────────────────────────────────────────────────
$hist = $db->prepare("SELECT * FROM REGISTROS WHERE ID_Empleado=? ORDER BY Fecha DESC, Hora_Entrada DESC LIMIT 30");
$hist->execute([$empId]);
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ── Sidebar info card (contrato + jornada) ── */
  .info-card { background:rgba(255,255,255,.08); border-radius:12px; padding:14px; margin-top:16px; }
  .info-card .ic-title { font-size:.7rem; font-weight:700; letter-spacing:.08em; text-transform:uppercase; color:rgba(255,255,255,.4); margin-bottom:8px; }
  .info-card .ic-row { display:flex; align-items:center; gap:8px; color:rgba(255,255,255,.75); font-size:.82rem; margin-bottom:5px; }
  .info-card .ic-row svg { width:13px; fill:currentColor; flex-shrink:0; opacity:.7; }
  .ic-dot { width:7px; height:7px; border-radius:50%; flex-shrink:0; }
  .ic-dot.verde   { background:#2ecc71; box-shadow:0 0 0 3px rgba(46,204,113,.2); }
  .ic-dot.amarillo{ background:#f39c12; box-shadow:0 0 0 3px rgba(243,156,18,.2); }
  .ic-dot.rojo    { background:#e74c3c; box-shadow:0 0 0 3px rgba(231,76,60,.2); }
  .dias-chips { display:flex; flex-wrap:wrap; gap:4px; margin-top:6px; }
  .dia-chip { padding:2px 8px; border-radius:5px; font-size:.7rem; font-weight:600; background:rgba(156,213,255,.15); color:var(--azul-claro); }
  .dia-chip.hoy { background:rgba(156,213,255,.35); color:white; }

  /* ── Main ── */
  .main { margin-left:260px; padding:36px 40px; min-height:100vh; }
  ...
  /* ── Reloj widget ── */
  .clock-widget { background:var(--azul-oscuro); border-radius:20px; padding:28px 32px; margin-bottom:20px; display:flex; align-items:center; justify-content:space-between; gap:20px; color:white; position:relative; overflow:hidden; }
  .clock-widget::before { content:''; position:absolute; right:-40px; top:-40px; width:200px; height:200px; border-radius:50%; background:rgba(122,170,206,.15); }
  .clock-time { font-family:'DM Serif Display',serif; font-size:3.5rem; line-height:1; letter-spacing:-.02em; }
  ...
  .action-btn { padding:14px 28px; border:none; border-radius:14px; font-family:'DM Sans',sans-serif; font-size:.95rem; font-weight:600; cursor:pointer; transition:all .2s; display:flex; align-items:center; gap:8px; }
  .btn-entrada { background:var(--azul-claro); color:var(--texto); }
  .btn-entrada:hover:not(:disabled) { filter:brightness(1.08); transform:translateY(-2px); }
  .btn-salida { background:rgba(255,255,255,.15); color:white; border:1.5px solid rgba(255,255,255,.3); }
  ...
  /* ── Alerta contrato/jornada ── */
  .alerta-strip { border-radius:12px; padding:12px 16px; margin-bottom:16px; display:flex; align-items:center; gap:10px; font-size:.86rem; font-weight:500; }
  .alerta-strip svg { width:16px; fill:currentColor; flex-shrink:0; }
  .alerta-strip.advertencia { background:rgba(192,57,43,.08); border:1.5px solid rgba(192,57,43,.2); color:#7d1e13; }
  .alerta-strip.info        { background:rgba(53,88,114,.07); border:1.5px solid rgba(53,88,114,.15); color:var(--azul-oscuro); }

  /* ── Info jornada ── */
  .jornada-banner { background:white; border-radius:14px; padding:16px 20px; margin-bottom:16px; box-shadow:var(--sombra); display:flex; align-items:center; gap:20px; flex-wrap:wrap; }
  .jornada-item { display:flex; align-items:center; gap:8px; }
  .jornada-item svg { width:16px; fill:var(--azul-oscuro); opacity:.5; }
  .jornada-label { font-size:.72rem; font-weight:600; text-transform:uppercase; letter-spacing:.05em; color:var(--azul-oscuro); opacity:.5; display:block; }
  .jornada-value { font-size:.92rem; font-weight:600; color:var(--texto); }

  /* ── Stats ── */
  .stats-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:16px; margin-bottom:20px; }
  .stat-card { background:white; border-radius:16px; padding:22px 24px; box-shadow:var(--sombra); }
  .stat-card .stat-label { font-size:.78rem; font-weight:600; letter-spacing:.06em; text-transform:uppercase; color:var(--azul-oscuro); opacity:.55; margin-bottom:8px; }
  .stat-card .stat-value { font-family:'DM Serif Display',serif; font-size:2rem; color:var(--texto); line-height:1; }
  .stat-card .stat-sub { font-size:.82rem; color:var(--azul-oscuro); opacity:.5; margin-top:4px; }

  /* ── Estado de hoy ── */
  .today-card { background:white; border-radius:16px; padding:22px 24px; box-shadow:var(--sombra); margin-bottom:20px; display:flex; align-items:center; gap:16px; }
  .status-dot { width:14px; height:14px; border-radius:50%; flex-shrink:0; }
  .status-dot.verde    { background:var(--verde);   box-shadow:0 0 0 4px rgba(39,174,96,.15); }
  .status-dot.amarillo { background:var(--naranja); box-shadow:0 0 0 4px rgba(243,156,18,.15); }
  ...
  /* ── Historial ── */
  .section-title { font-family:'DM Serif Display',serif; font-size:1.25rem; color:var(--texto); margin-bottom:16px; }
  .table-wrap { background:white; border-radius:16px; box-shadow:var(--sombra); overflow:hidden; }
  table { width:100%; border-collapse:collapse; }
  thead th { padding:14px 20px; text-align:left; font-size:.75rem; font-weight:600; letter-spacing:.07em; text-transform:uppercase; color:var(--azul-oscuro); opacity:.55; background:rgba(53,88,114,.04); border-bottom:1px solid rgba(53,88,114,.08); }
  tbody td { padding:14px 20px; font-size:.9rem; color:var(--texto); border-bottom:1px solid rgba(53,88,114,.06); }
  tbody tr:last-child td { border-bottom:none; }
  tbody tr:hover { background:rgba(53,88,114,.03); }
  .badge { display:inline-block; padding:3px 10px; border-radius:6px; font-size:.75rem; font-weight:600; }
  ...
  /* ── Toast ── */
  .toast { position:fixed; bottom:32px; right:32px; background:var(--texto); color:white; padding:14px 20px; border-radius:14px; font-size:.92rem; font-weight:500; box-shadow:0 8px 32px rgba(0,0,0,.2); transform:translateY(80px); opacity:0; transition:all .35s cubic-bezier(.22,1,.36,1); z-index:999; max-width:320px; display:flex; align-items:center; gap:10px; }
...
<aside class="sidebar">
  ...
  <!-- R13: estado contrato en sidebar -->
  <div class="info-card">
    <div class="ic-title">Mi contrato</div>
    <?php if ($contratoVigente): ?>
    <div class="ic-row">
      <span class="ic-dot verde"></span>
      <span>Vigente · <?= htmlspecialchars($contratoVigente['Tipo_Contrato'] ?? 'N/A') ?></span>
    </div>
    <div class="ic-row">
      <svg viewBox="0 0 24 24"><path d="M11.8 10.9c-2.27-.59-3-1.2-3-2.15 0-1.09 1.01-1.85 2.7-1.85 1.78 0 2.44.85 2.5 2.1h2.21c-.07-1.72-1.12-3.3-3.21-3.81V3h-3v2.16c-1.94.42-3.5 1.68-3.5 3.61 0 2.31 1.91 3.46 4.7 4.13 2.5.6 3 1.48 3 2.41 0 .69-.49 1.79-2.7 1.79-2.06 0-2.87-.92-2.98-2.1h-2.2c.12 2.19 1.76 3.42 3.68 3.83V21h3v-2.15c1.95-.37 3.5-1.5 3.5-3.55 0-2.84-2.43-3.81-4.7-4.4z"/></svg>
      <?= $contratoVigente['Salario_Pactado'] ? '$ '.number_format($contratoVigente['Salario_Pactado'],0,',','.') : 'Salario N/D' ?>
    </div>
    <?php else: ?>
    <div class="ic-row">
      <span class="ic-dot rojo"></span>
      <span style="color:rgba(255,100,80,.9)">Sin contrato vigente</span>
    </div>
    <?php endif; ?>
  </div>

  <!-- R17: jornada y días laborales en sidebar -->
  <div class="info-card">
    <div class="ic-title">Mi jornada</div>
    <div class="ic-row">
      <svg viewBox="0 0 24 24"><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67V7z"/></svg>
      <?= $emp['Hora_Inicio_Jornada'] ? substr($emp['Hora_Inicio_Jornada'],0,5).' – '.substr($emp['Hora_Fin_Jornada'],0,5) : 'Horario no definido' ?>
    </div>
    <?php if (!empty($diasLaborales)): ?>
    <div class="dias-chips" style="margin-top:8px;">
      <?php
        $hoyNombre = ['Sunday'=>'Domingo','Monday'=>'Lunes','Tuesday'=>'Martes','Wednesday'=>'Miércoles','Thursday'=>'Jueves','Friday'=>'Viernes','Saturday'=>'Sábado'][date('l')];
        foreach ($diasLaborales as $dia):
          $esHoy = ($dia === $hoyNombre);
      ?>
      <span class="dia-chip <?= $esHoy?'hoy':'' ?>"><?= htmlspecialchars(substr($dia,0,3)) ?></span>
      <?php endforeach; ?>
    </div>
    <?php else: ?>
    <div style="color:rgba(255,255,255,.35);font-size:.78rem;margin-top:6px;">Días no configurados</div>
    <?php endif; ?>
  </div>
...
<main class="main">
  ...
  <?php if (!$contratoVigente && $emp['Estado_Empleado']==='Activo'): ?>
  <!-- R13: alerta si empleado activo sin contrato -->
  <div class="alerta-strip advertencia">
    <svg viewBox="0 0 24 24"><path d="M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z"/></svg>
    <span>No tienes un contrato vigente registrado. Contacta a Recursos Humanos.</span>
  </div>
  <?php endif; ?>

  <!-- Jornada banner (R17) -->
  <?php if ($emp['Hora_Inicio_Jornada']): ?>
  <div class="jornada-banner">
    <div class="jornada-item">
      <svg viewBox="0 0 24 24"><path d="M11.99 2C6.47 2 2 6.48 2 12s4.47 10 9.99 10C17.52 22 22 17.52 22 12S17.52 2 11.99 2zM12 20c-4.42 0-8-3.58-8-8s3.58-8 8-8 8 3.58 8 8-3.58 8-8 8zm.5-13H11v6l5.25 3.15.75-1.23-4.5-2.67V7z"/></svg>
      <div>
        <span class="jornada-label">Jornada asignada</span>
        <span class="jornada-value"><?= substr($emp['Hora_Inicio_Jornada'],0,5) ?> – <?= substr($emp['Hora_Fin_Jornada'],0,5) ?></span>
      </div>
    </div>
    <?php if (!empty($diasLaborales)): ?>
    <div class="jornada-item">
      <svg viewBox="0 0 24 24"><path d="M17 12h-5v5h5v-5zM16 1v2H8V1H6v2H5c-1.11 0-1.99.9-1.99 2L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2h-1V1h-2zm3 18H5V8h14v11z"/></svg>
      <div>
        <span class="jornada-label">Días laborales</span>
        <span class="jornada-value"><?= implode(', ', array_map(fn($d)=>substr($d,0,3), $diasLaborales)) ?></span>
      </div>
    </div>
    <?php endif; ?>
    <div class="jornada-item" style="margin-left:auto;">
      <svg viewBox="0 0 24 24"><path d="M11.8 10.9c-2.27-.59-3-1.2-3-2.15 0-1.09 1.01-1.85 2.7-1.85 1.78 0 2.44.85 2.5 2.1h2.21c-.07-1.72-1.12-3.3-3.21-3.81V3h-3v2.16c-1.94.42-3.5 1.68-3.5 3.61 0 2.31 1.91 3.46 4.7 4.13 2.5.6 3 1.48 3 2.41 0 .69-.49 1.79-2.7 1.79-2.06 0-2.87-.92-2.98-2.1h-2.2c.12 2.19 1.76 3.42 3.68 3.83V21h3v-2.15c1.95-.37 3.5-1.5 3.5-3.55 0-2.84-2.43-3.81-4.7-4.4z"/></svg>
      <div>
        <span class="jornada-label">Salario base cargo</span>
        <span class="jornada-value"><?= $emp['Salario_Base'] ? '$ '.number_format($emp['Salario_Base'],0,',','.') : 'N/D' ?></span>
      </div>
    </div>
  </div>
  <?php endif; ?>
  ...
  <div class="today-card">
    ...
    <div class="today-times">
      <div class="time-block">
        ...
        <div class="t-value"><?= substr($registroHoy['Hora_Entrada'],0,5) ?></div>
      </div>
      <?php if ($haySalida): ?>
      <div class="time-block">
